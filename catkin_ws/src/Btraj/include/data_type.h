...
struct Cube {
  ...
  */
  ...
  // create a inscribe cube of a ball using the center point and the radius of
  // the ball
  ...
  Cube() {
    center = Eigen::VectorXd::Zero(3);     // 立方体中心点 [x, y, z]
    vertex = Eigen::MatrixXd::Zero(8, 3);  // 8个顶点坐标 (8×3矩阵)

    valid = true;   // 有效性标志
    t = 0.0;        // 时间分配
    box.resize(3);  // xyz三轴边界范围容器
  }

...
double _x_local_size, _y_local_size,
    _z_local_size;  // 局部地图尺寸（米）- 提高计算效率
...
CollisionMapGrid* collision_map_local =
    new CollisionMapGrid();  // 局部碰撞检测地图（提高效率）
...
void rcvWaypointsCallback(
    const nav_msgs::Path& wp);  // 接收目标点，触发轨迹规划
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2&
                               pointcloud_map);  // 接收点云地图，更新碰撞检测
void rcvOdometryCallbck(
    const nav_msgs::Odometry odom);  // 接收里程计，更新机器人状态
...
vector<pcl::PointXYZ> pointInflate(
    pcl::PointXYZ pt);  // 点云膨胀：为UAV添加安全边界
...
void visBezierTrajectory(MatrixXd polyCoeff,
                         VectorXd time);  // 显示最终Bezier轨迹

// ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
// ░░  4.4 走廊生成相关函数 - 安全飞行区域构建
// ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
// 走廊：无碰撞的立方体区域，用于约束轨迹优化
pair<Cube, bool> inflateCube(Cube cube,
                             Cube lstcube);  // 立方体膨胀：扩大至最大无碰撞尺寸
...
vector<Cube> corridorGeneration(vector<Vector3d> path_coord,
                                vector<double> time);  // 生成走廊（带时间）
vector<Cube> corridorGeneration(
    vector<Vector3d> path_coord);  // 生成走廊（不带时间）

// ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
// ░░  4.5 时间分配相关函数 - 轨迹时间优化
// ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
void sortPath(vector<Vector3d>& path_coord,
              vector<double>& time);  // 路径排序和清理
void timeAllocation(vector<Cube>& corridor,
                    vector<double> time);     // 时间分配（带初始时间）
void timeAllocation(vector<Cube>& corridor);  // 时间分配（自动计算）

// ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
// ░░  4.6 Bezier轨迹计算函数 - 轨迹状态查询和封装
// ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
VectorXd getStateFromBezier(
    const MatrixXd& polyCoeff,
    double t_now,
    int seg_now);  // 获取全状态（位置+速度+加速度+Jerk）
Vector3d getPosFromBezier(const MatrixXd& polyCoeff,
                          double t_now,
                          int seg_now);                // 获取位置状态
quadrotor_msgs::PolynomialTrajectory getBezierTraj();  // 封装轨迹为ROS消息

// ████████████████████████████████████████████████████████████████████████████████
// ██ ██ ██                           第5部分: 回调函数实现 ██ ██ Callback
...
Vector3d _local_origin;  // 局部地图原点（全局变量，用于地图更新）
...
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2& pointcloud_map) {
  ...
  // 设置膨胀点云的PCL属性
  cloud_inflation.width =
      cloud_inflation.points.size();          // 点云宽度（点的数量）
...
bool checkExecTraj() {
  ...
  _stop_traj_vis.type = _check_traj_vis.type =
      visualization_msgs::Marker::SPHERE_LIST;  // 球列表类型
  _stop_traj_vis.action = _check_traj_vis.action =
      visualization_msgs::Marker::ADD;  // 添加操作
...
void trajPlanning() {
  ...
  if (_trajectoryGenerator.BezierPloyCoeffGeneration(
          ...
          _bezier_coeff) == -1) {
    ROS_WARN(
        "Cannot find a feasible and optimal solution, somthing wrong with the "
        "mosek solver");
...
int main(int argc, char** argv) {
  ...
  _odom_sub =
      nh.subscribe("odometry", 1, rcvOdometryCallbck);  // 订阅里程计数据
  ...
  // 可视化相关发布者
  _inf_map_vis_pub = nh.advertise<sensor_msgs::PointCloud2>(
      "vis_map_inflate", 1);  // 膨胀地图可视化
  _local_map_vis_pub = nh.advertise<sensor_msgs::PointCloud2>(
      "vis_map_local", 1);  // 局部地图可视化
  _traj_vis_pub = nh.advertise<visualization_msgs::Marker>("trajectory_vis",
                                                           1);  // 轨迹可视化
  _corridor_vis_pub = nh.advertise<visualization_msgs::MarkerArray>(
      "corridor_vis", 1);  // 走廊可视化
  _fm_path_vis_pub = nh.advertise<visualization_msgs::MarkerArray>(
      "path_vis", 1);  // Fast Marching路径可视化
  _grid_path_vis_pub = nh.advertise<visualization_msgs::MarkerArray>(
      "grid_path_vis", 1);  // A*网格路径可视化
  _nodes_vis_pub = nh.advertise<visualization_msgs::Marker>(
      "expanded_nodes_vis", 1);  // A*搜索节点可视化
  _checkTraj_vis_pub = nh.advertise<visualization_msgs::Marker>(
      "check_trajectory", 1);  // 轨迹检查可视化
  _stopTraj_vis_pub = nh.advertise<visualization_msgs::Marker>(
      "stop_trajectory", 1);  // 紧急停止轨迹可视化

  // 控制指令发布者
  _traj_pub = nh.advertise<quadrotor_msgs::PolynomialTrajectory>(
      "trajectory", 10);  // 轨迹指令发布
  ...
  // 算法内部参数
  nh.param("planning/max_inflate", _max_inflate_iter,
           100);                                      // 立方体膨胀最大迭代次数
  nh.param("planning/step_length", _step_length, 2);  // 膨胀步长（网格单位）
  nh.param("planning/cube_margin", _cube_margin, 0.2);  // 立方体安全边界（米）

  // 安全检查参数
  nh.param("planning/check_horizon", _check_horizon,
           10.0);  // 碰撞检测前瞻时间（秒）
  nh.param("planning/stop_horizon", _stop_horizon,
           5.0);  // 紧急停止判断时间（秒）

  // 优化约束开关
  nh.param("planning/is_limit_vel", _is_limit_vel, false);  // 是否启用速度约束
  nh.param("planning/is_limit_acc", _is_limit_acc,
           false);  // 是否启用加速度约束

  // 算法选择参数
  nh.param("planning/is_use_fm", _is_use_fm,
           true);  // 是否使用Fast Marching（否则使用A*）

  // 轨迹优化参数
  nh.param("optimization/min_order", _minimize_order,
           3.0);  // 优化目标阶数（snap最小化）
  ...
  Vector3i LOSIZE(_max_local_x_id, _max_local_y_id,
                  _max_local_z_id);  // 局部地图尺寸
  ...
  // 初始化全局碰撞检测地图
  Translation3d origin_translation(_map_origin(0), _map_origin(1),
                                   0.0);            // 平移变换
...
quadrotor_msgs::PolynomialTrajectory getBezierTraj() {
  ...
  // 设置轨迹基本属性
  traj.action =
      quadrotor_msgs::PolynomialTrajectory::ACTION_ADD;  // 设置为添加新轨迹
...
VectorXd getStateFromBezier(const MatrixXd& polyCoeff,
                            double t_now,
                            int seg_now) {
